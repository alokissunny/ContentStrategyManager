...
function Hero({ onGo }) {
  return (
    <section id="top" style={{ background: LS_BG, overflow: 'hidden' }}>
      <div style={{ ...LSC, position: 'relative', padding: '56px 48px 64px', display: 'grid', gridTemplateColumns: '0.82fr 1.18fr', gap: 24, alignItems: 'center' }}>
        <div style={{ position: 'relative', zIndex: 2 }}>
          <h1 style={{ fontFamily: LS_DISPLAY, fontWeight: 800, fontSize: 'clamp(40px, 4.4vw, 62px)', lineHeight: 0.98, letterSpacing: '-0.02em', margin: 0 }}>
        ...
        <div style={{ minHeight: 420, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 -80px 0 0' }}><SignalTerrain /></div>
      </div>
    </section>
  );
}
...
function LogoStrip() {
  ...
  return (
    <section style={{ background: LS_BG, borderTop: `1px solid ${LS_BORDER}`, borderBottom: `1px solid ${LS_BORDER}` }}>
      <div style={{ ...LSC, padding: '28px 48px', display: 'flex', alignItems: 'center', gap: 40, flexWrap: 'wrap' }}>
...
function Problem() {
  ...
  return (
    <section style={{ background: LS_BG, borderBottom: `1px solid ${LS_BORDER}` }}>
      <div style={{ ...LSC, padding: '72px 48px', display: 'grid', gridTemplateColumns: '0.74fr 1.26fr', gap: 56, alignItems: 'start' }}>
        <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 0 }}>
          {items.map(([ico, t, b], i) => (
            <div key={i} style={{ padding: '24px 26px', textAlign: 'center', borderLeft: i % 3 !== 0 ? `1px solid ${LS_BORDER}` : 'none', borderTop: i >= 3 ? `1px solid ${LS_BORDER}` : 'none' }}>
      ...
      <div style={{ ...LSC, padding: '0 48px 56px' }}>
        <div style={{ border: `1px solid ${LS_BORDER}`, borderRadius: 12, background: LS_SURFACE, padding: '22px 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 40 }}>
          ...
          <svg width="340" height="40" viewBox="0 0 340 40" fill="none" style={{ flexShrink: 0 }}>
...
function Philosophy() {
  ...
  return (
    <section id="philosophy" style={{ background: LS_BG, borderBottom: `1px solid ${LS_BORDER}` }}>
      <div style={{ ...LSC, padding: '72px 48px', display: 'grid', gridTemplateColumns: '0.74fr 1.26fr', gap: 56, alignItems: 'center' }}>
        <div>
        ...
        <div style={{ border: `1px solid ${LS_BORDER}`, borderRadius: 14, background: LS_SURFACE, padding: '34px 32px' }}>
  ...
  return (
    <section style={{ background: LS_BG, borderBottom: `1px solid ${LS_BORDER}` }}>
      <div style={{ ...LSC, padding: '72px 48px', display: 'grid', gridTemplateColumns: '0.74fr 1.26fr', gap: 56, alignItems: 'start' }}>
        ...
        <div style={{ border: `1px solid ${LS_BORDER}`, borderRadius: 14, background: LS_SURFACE, display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }}>
          {cards.map((c, i) => (
            <div key={i} style={{ padding: '22px 20px', borderLeft: i ? `1px solid ${LS_BORDER}` : 'none', display: 'flex', flexDirection: 'column' }}>
          ...
          {steps.map(([n, t, b], i) => (
            <React.Fragment key={n}>
              <div style={{ textAlign: 'center', flex: 1 }}>
                <div style={{ display: 'flex', justifyContent: 'center' }}><IcoRings size={44} /></div>
                <div style={{ fontFamily: LS_FONT, fontSize: 11, color: LS_MUTED, margin: '14px 0 4px' }}>{n}</div>
                ...
                <p style={{ fontFamily: LS_FONT, fontSize: 12.5, lineHeight: 1.5, color: LS_T2, margin: '0 auto', maxWidth: 130 }}>{b}</p>
              </div>
              {i < 2 && <Glyph name="arrow-right" size={20} color={LS_MUTED} style={{ flexShrink: 0 }} />}
            </React.Fragment>
          ))}
          <div style={{ borderLeft: `1px solid ${LS_BORDER}`, paddingLeft: 24, marginLeft: 8, flexShrink: 0, width: 150 }}>
...
function Closing({ onGo }) {
  ...
  return (
    <section style={{ background: LS_BG, position: 'relative', overflow: 'hidden' }}>
      <div style={{ ...LSC, padding: '80px 48px 72px', textAlign: 'center', position: 'relative', zIndex: 2 }}>
