/*
 * The slide engine — how a generated slide is allowed to look.
 *
 * ── THIS WAS `visualbrand.js` (Leon, Aug 7) ───────────────────────────────
 *
 * The Visual Brand was an entity with two screens; it is removed (decision
 * 511), because its Visual Style tab set the palette and the faces, which is
 * what Library Settings does. This module is NOT that entity. It is the
 * arithmetic the weekly plan renders every slide through — the ground, the
 * roles, and whether an empty slide can honestly be generated at all.
 * `YourWeek` cannot run without it.
 *
 * It is renamed rather than deleted so nobody goes looking for a page. Two
 * things to know when reading the rest of this file:
 *
 *   · The three COLOURS are Library Settings' now — `rolesOf` checks the
 *     identity first, and `brandStyle.colours` was migrated away in store.js.
 *   · The LAYOUTS are `data/layouts.js`', drawn by `LayoutArt.jsx`. This file
 *     held a second set of 18 with its own taxonomy and renderer; decision 517
 *     removed it, and the Visual Library and the plan are now one system.
 *
 * The original note follows, and its rules still hold.
 *
...
/* ── THE PLAN'S OWN LAYOUT SET IS GONE (Leon, Aug 7 — decision 517) ───────
 *
 * `LAYOUT_GROUPS`, `LAYOUTS` (18), `layoutsOf`, `groupForRole`, `layoutGroup`
 * and `refsForGroup` lived here: a second set of layouts, with a second
 * taxonomy and a second renderer, beside the Visual Library's own. Two systems
 * describing one thing is how the Library and Plans came to look like separate
 * features rather than two views of one design system.
 *
 * There is one set now — `src/data/layouts.js` — one renderer (`LayoutArt.jsx`)
 * and one taxonomy (the library's six categories, mapped from a slide's job by
 * `catForRole`). `shots` and `needs` are derived from the composition there
 * rather than written twice. This closes TECH_DEBT 34.
 *
 * What stayed in this file is what it is actually for: the ground a slide is
 * drawn on, the colour roles, and whether a slide can honestly be generated.
 */
...
/* ── LIBRARY SETTINGS OWNS THE THREE COLOURS (Leon, Aug 7) ────────────────
 * `style.colours` was the Visual Style tab's copy of them. That page is gone
 * and its values were migrated into `libraryEdits.palette` on load (store.js),
 * so the identity is checked FIRST and `style.colours` only survives as the
 * fallback for a session mid-migration. One place sets these now. */
export function rolesOf(style, palette, identity) {
  const id = identity || {};
  if (id.fg || id.accent || id.ground) {
    return {
      primary: id.fg || style.colours?.primary || '#16161a',
      accent: id.accent || style.colours?.accent || '#8d7157',
      neutral: id.ground || style.colours?.neutral || '#f5f0e8',
    };
  }
