...
export default function StageFunnel({ stages, focusKey, confidence = 'High confidence' }) {
  ...
  return (
    <div style={{ background: LS_SURFACE, border: `1px solid ${LS_BORDER}`, borderRadius: 16, padding: '24px 28px 28px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', marginTop: 36 }}>
        {stages.map((s, i) => {
          ...
          return (
            <React.Fragment key={s.key}>
              {i > 0 && (
                <div style={{ display: 'flex', alignItems: 'center', paddingTop: isFocus || stages[i - 1].key === focusKey ? 30 : 30 }}>
                  <Glyph name="chevron-right" size={18} color={LS_MUTED} />
                </div>
              )}
